...
const ACCENT = "#feb800";
const CONFETTI_COLORS = [ACCENT, "#ffd54f", "#ffc107", "#f9a825", "#ffe082", "#fff59d"];

function burstYellowConfetti(x, y) {
  const count = 132;
  const bursts = 3;
...
export function MagneticButton({
  ...
  className = "",
  onClick,
  "aria-label": ariaLabel = "Magnetic button",
  ...props
}) {
  ...
  const handleClick = (e) => {
    ...
    if (btn) {
      ...
      burstYellowConfetti(rect.left + rect.width / 2, rect.top + rect.height / 2);
    }
  ...
  return (
    <div
      ref={wrapRef}
      className={`mag-wrap${hovered ? " mag-wrap--hover" : ""}`}
    >
      <div ref={motionRef} className="mag-btn-motion">
        <div className="mag-btn-scale">
          <button
            ...
            onClick={handleClick}
            aria-label={hovered ? "Click me" : ariaLabel}
            {...props}
          >
