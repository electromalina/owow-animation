...
import { ControlsSidebar } from "@/src/components/ControlsSidebar";
import type { Animation, AnimationState } from "@/src/data/animations";

function buildDefaultState(animation: Animation): AnimationState {
  return {
    platform: "web",
    params: Object.fromEntries(
      animation.params.map((p) => [p.key, p.defaultValue])
    ),
  };
}
...
export function DetailClient({ animation }: { animation: Animation }) {
  ...
  function handleReset() {
    setState(buildDefaultState(animation));
  }
      ...
      >
        ...
        <article className="flex min-w-0 flex-col gap-14">
          ...
          <PreviewCard
            ...
            strength={state.params.strength ?? 1.2}
            radius={state.params.radius ?? 120}
            stiffness={state.params.stiffness ?? 0.18}
            damping={state.params.damping ?? 0.7}
          />
        ...
        {/* RIGHT — Nikita's controls sidebar */}
        <ControlsSidebar
          animation={animation}
          state={state}
